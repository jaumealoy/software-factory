...
export interface FilesRoutesOptions {
  db: Db;
  /** Resolves the repo root for a project and optional folder id. */
  resolveRoot?: (projectId: string, folderId?: string) => string | null;
}
...
export const filesRoutes: FastifyPluginAsync<FilesRoutesOptions> = async (fastify, options) => {
  ...
  fastify.get<{
    ...
    Querystring: { path?: string; folderId?: string };
  }>("/api/projects/:projectId/files", async (request, reply) => {
    const root = resolveRoot(request.params.projectId, request.query.folderId);
    if (!root) {
      return request.query.folderId
        ? reply.code(404).send({ error: "Unknown folder for this project" })
        : { exists: false, path: ".", entries: [], message: "No repository configured" };
    }
    try {
  ...
  fastify.get<{
    Params: { projectId: string };
    Querystring: { path?: string; folderId?: string };
  }>("/api/projects/:projectId/files/content", async (request, reply) => {
